...
def parse_bibtex_entry(
    ...
    for key in entry.keys():
        if key in ('author','title','abstract'):
            lx = entry[key]
            lx = lx.replace('-\n','-')
            lx = lx.replace('\n',' ')
            lx = lx.replace(r'å',r'\ra')
            lx = lx.replace(r'i̊',r'\rai')
            lx = lx.replace(r'\raisebox-0.5ex',r'')
            lx = lx.replace(r'\ensuremath','')
            lx = lx.replace(r' _',r'_')
            lx = lx.replace(' /','/')
            lx = lx.replace(r' $_',r'$_')
            lx = lx.replace(r' \text',r'\text')
            lx = lx.replace(r'rm ',r'')
            lx = lx.replace(r'mathrm',r'')
            # lx = lx.replace(r'\%',r'%')
            lx = lx.replace(r'ł',r'\l')
            lx = lx.replace(r'\log',r'log')
            lx = lx.replace(r'ø',r'\o')
            lx = lx.replace(r'\odot',r'⊙')

            # nlx=ln2t.latex_to_text(lx)
            nlx = latex2markdown.LaTeX2Markdown(lx).to_markdown()
            nlx = nlx.replace('$','')
            entry[key]=nlx
    ...
    page.yaml["publication_types"] = [str(pubtype.value)]
    ...
    if "booktitle" in entry:
        ...
        publication = ""
    if publication == '*apj*': publication = '*The Astrophysical Journal*'
    if publication == '*apjl*': publication = '*The Astrophysical Journal Letters*'
    if publication == '*apjs*': publication = '*The Astrophysical Journal Supplement Series*'
    if publication == '*mnras*': publication = '*Monthly Notices of the Royal Astronomical Society*'
